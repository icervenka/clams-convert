...
def parse_classic(csv, column_specs):
    ...
    data.loc[:, date_time_colstring] = data.loc[:, date_time_colstring].dt.strftime(clams_const.output_time_format)
...
def parse_tse(csv, column_specs):
    ...
    if tse_start_line == None:
        sys.exit("TSE format not recognized. Please make sure that it contains the header with following string: " +
                 clams_const.patterns_tse['start'])
    ...
    data.loc[:, "date_time"] = data.loc[:, "date_time"].dt.strftime(clams_const.output_time_format)
    data.loc[:, "date_time"] = data.loc[:, "date_time"].astype(str)

    data.loc[:, col_finder['light']] = data.loc[:, col_finder['light']].apply(lambda y: 1 if y > 50 else 0)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Concatenate CSV files from CLAMS to one file for analysis by clams-vis")
    ...
    parser.add_argument('-s', '--system', help="System which was used to record the data", choices=("classic", "tse"),
                        default="classic")
    parser.add_argument('-d', '--date', help="Date-Time format used for export. Accepts strftime format strings",
                        default="%d-%m-%Y %H:%M:%S")
    ...
    if table_format == "tse":
        if len(file_list) > 1:
            sys.exit(
                "Too many csv files to process. For TSE system type only one csv file has to be present in the "
                "directory.")
