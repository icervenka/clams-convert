...
def parse_tse(csv, column_specs):
    ...
    data.loc[:,"date-time"] = data.loc[:,"date-time"].dt.strftime("%d-%m-%Y %H:%M:%S")
    ...
    ser = []
    interval_count = list(data.groupby(col_finder['subject']).count()["date-time"])
...
def main():
    ...
    # check if the column specification file for diffrent systems exists and import it
